...
export default class SpriteForgeService extends Service {
  ...
  drawing = false; ds = null; dc = null;
  panning = false; panA = null; panS = null;
  moving = false; movA = null;
  scaleMode = 'fit';
  scaleAlign = 'mc';
  scCache = null;
  pvZ = 1;
  ...
  fitZoom() {
    ...
    const aw = carea.clientWidth - 40, ah = carea.clientHeight - 40;
    const z = this.clamp(Math.min(aw / this.img.width, ah / this.img.height), 0.1, 1);
  ...
  loadJSON(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        this.frames = new TrackedArray(); this.fc = 0; this.sel = -1;
        if (data.frames && !Array.isArray(data.frames)) {
          Object.entries(data.frames).forEach(([name, v]) => {
            const f = v.frame || v;
            this.frames.push({ id: this.fc++, name, x: f.x || 0, y: f.y || 0, w: f.w || f.width || 64, h: f.h || f.height || 64 });
          });
        } else if (Array.isArray(data.frames)) {
          data.frames.forEach((f, i) => {
            const r = f.frame || f;
            this.frames.push({ id: this.fc++, name: f.filename || f.name || 'frame_' + i, x: r.x || 0, y: r.y || 0, w: r.w || r.width || 64, h: r.h || r.height || 64 });
          });
        } else if (data.sprites) {
          data.sprites.forEach((f, i) => this.frames.push({ id: this.fc++, name: f.name || 'frame_' + i, x: f.x || 0, y: f.y || 0, w: f.w || f.width || 64, h: f.h || f.height || 64 }));
        }
        this.redraw();
  ...
  setMode(m) {
    this.mode = m;
    const cursors = { sel: 'default', drw: 'crosshair', mov: 'grab', pan: 'grab' };
  ...
  cPos(e) {
    ...
    return { x: Math.round((e.clientX - r.left) / this.zoom), y: Math.round((e.clientY - r.top) / this.zoom) };
  }
  ...
  handleMouseDown(e) {
    ...
    const p = this.cPos(e);
    if (this.mode === 'pan') {
      this.panning = true; this.panA = { x: e.clientX, y: e.clientY };
      const carea = document.getElementById('carea');
      this.panS = { x: carea.scrollLeft, y: carea.scrollTop };
      if (this._canvas) this._canvas.style.cursor = 'grabbing';
      return;
    }
    if (this.mode === 'drw') { this.drawing = true; this.ds = { ...p }; this.dc = { ...p }; return; }
    if (this.mode === 'mov') {
      const i = this.frameAt(p.x, p.y);
      if (i >= 0) { this.sel = i; this.moving = true; this.movA = { x: p.x - this.frames[i].x, y: p.y - this.frames[i].y }; if (this._canvas) this._canvas.style.cursor = 'grabbing'; this.redraw(); }
      return;
    }
    if (this.mode === 'sel') { this.sel = this.frameAt(p.x, p.y); this.redraw(); }
  }

  handleMouseMove(e) {
    if (!this.img || !this._canvas) return;
    const r = this._canvas.getBoundingClientRect();
    const cx = Math.round((e.clientX - r.left) / this.zoom), cy = Math.round((e.clientY - r.top) / this.zoom);
    ...
    if (this.drawing && this.ds) { this.dc = { x: cx, y: cy }; this.redraw(); return; }
    if (this.moving && this.sel >= 0) {
      const p = this.cPos(e), f = this.frames[this.sel];
      ...
      if (document.getElementById('sb5')) {
        document.getElementById('sb5').innerHTML = `Sel: <b>${f.name}</b> ${f.w}×${f.h}`;
      }
      this.redraw();
    }
  }
  ...
  handleMouseUp() {
    if (this.panning) { this.panning = false; this.panA = null; if (this._canvas) this._canvas.style.cursor = 'grab'; return; }
    if (this.moving) { this.moving = false; if (this._canvas) this._canvas.style.cursor = 'grab'; return; }
    if (this.drawing && this.ds && this.dc) {
      this.drawing = false;
      const x = Math.min(this.ds.x, this.dc.x), y = Math.min(this.ds.y, this.dc.y);
      const w = Math.abs(this.dc.x - this.ds.x), h = Math.abs(this.dc.y - this.ds.y);
      if (w > 3 && h > 3) {
      ...
      this.ds = null; this.dc = null; this.redraw();
    }
  }
  ...
  handleWheel(e) {
    if (e.ctrlKey || e.metaKey) {
      e.preventDefault();
      this.applyZoom(this.clamp(Math.round(this.zoom * 100) + (e.deltaY > 0 ? -10 : 10), 10, 800));
    }
  }
  ...
  handleKeyDown(e) {
    ...
    if (k === '+' || k === '=') this.applyZoom(this.clamp(Math.round(this.zoom * 100) + 25, 10, 800));
    if (k === '-') this.applyZoom(this.clamp(Math.round(this.zoom * 100) - 25, 10, 800));
    if ((k === 'delete' || k === 'backspace') && this.sel >= 0) this.delFrame(this.sel);
    if (k === 'escape') { this.sel = -1; this.redraw(); }
    if (this.sel >= 0 && !e.shiftKey) {
      const f = this.frames[this.sel];
      if (k === 'arrowleft') { f.x = Math.max(0, f.x - 1); this.redraw(); }
      if (k === 'arrowright') { f.x++; this.redraw(); }
      if (k === 'arrowup') { f.y = Math.max(0, f.y - 1); this.redraw(); }
      if (k === 'arrowdown') { f.y++; this.redraw(); }
    }
    if (this.sel >= 0 && e.shiftKey) {
      const f = this.frames[this.sel];
      if (k === 'arrowright') { f.w++; this.redraw(); }
      if (k === 'arrowleft') { f.w = Math.max(1, f.w - 1); this.redraw(); }
      if (k === 'arrowdown') { f.h++; this.redraw(); }
      if (k === 'arrowup') { f.h = Math.max(1, f.h - 1); this.redraw(); }
    }
  }
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      rd.onload = (ev) => {
        ...
        img.onload = () => {
          ...
          if (gW) gW.value = Math.max(16, Math.floor(this.img.width / 4));
          if (gH) gH.value = Math.max(16, Math.floor(this.img.height / 4));
          this.fc = 0; this.frames = new TrackedArray(); this.sel = -1;
          this.fitZoom();
        };
  ...
  redraw() {
    ...
    ctx.save(); ctx.scale(zoom, zoom); ctx.imageSmoothingEnabled = false;
    ...
    this.frames.forEach((f, i) => {
      ...
      ctx.strokeRect(f.x + 0.5 / zoom, f.y + 0.5 / zoom, f.w - 1 / zoom, f.h - 1 / zoom);
      ...
      if (s) {
        const hs = 5 / zoom; ctx.fillStyle = '#39e8b0';
    ...
    if (this.drawing && this.ds && this.dc) {
      const x = Math.min(this.ds.x, this.dc.x), y = Math.min(this.ds.y, this.dc.y);
      const w = Math.abs(this.dc.x - this.ds.x), h = Math.abs(this.dc.y - this.ds.y);
      ctx.strokeStyle = '#ffc94d'; ctx.lineWidth = 2 / zoom;
      ctx.setLineDash([6 / zoom, 3 / zoom]); ctx.strokeRect(x, y, w, h); ctx.setLineDash([]);
      ctx.fillStyle = 'rgba(255,201,77,.1)'; ctx.fillRect(x, y, w, h);
      ctx.font = `${Math.max(9, 11 / zoom)}px monospace`; ctx.fillStyle = '#ffc94d';
  ...
  @action
  addManual() {
    const x = +document.getElementById('nfx').value, y = +document.getElementById('nfy').value;
    const w = +document.getElementById('nfw').value, h = +document.getElementById('nfh').value;
    const name = document.getElementById('nfname').value || 'frame_' + this.fc;
    this.frames.push({ id: this.fc, name, x, y, w, h });
    document.getElementById('nfname').value = 'frame_' + (this.fc + 1);
    this.fc++;
  ...
  applyGrid() {
    ...
    const fw = +document.getElementById('gW').value || 64, fh = +document.getElementById('gH').value || 64;
    const ox = +document.getElementById('gOX').value || 0, oy = +document.getElementById('gOY').value || 0;
    const pfx = document.getElementById('gPfx').value || 'frame_';
    this.frames = new TrackedArray(); this.fc = 0; this.sel = -1;
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        this.frames.push({ id: this.fc, name: pfx + this.fc++, x: ox + c * fw, y: oy + r * fh, w: fw, h: fh });
      }
    }
    this.redraw();
  }
  ...
  @action
  runAutoDetect() {
    if (!this.img) { alert('Carga una imagen primero'); return; }
    ...
    const off = document.createElement('canvas'); off.width = this.img.width; off.height = this.img.height;
    off.getContext('2d').drawImage(this.img, 0, 0);
    const px = off.getContext('2d').getImageData(0, 0, this.img.width, this.img.height).data;
    const isBg = (x, y) => {
      const i = (y * this.img.width + x) * 4;
      if (px[i + 3] < 10) return true;
      return Math.abs(px[i] - bgR) <= tol && Math.abs(px[i + 1] - bgG) <= tol && Math.abs(px[i + 2] - bgB) <= tol;
    };
    const vis = new Uint8Array(this.img.width * this.img.height), blobs = [];
    for (let y = 0; y < this.img.height; y++) {
      for (let x = 0; x < this.img.width; x++) {
        const idx = y * this.img.width + x;
        if (vis[idx] || isBg(x, y)) continue;
        let mnX = x, mxX = x, mnY = y, mxY = y;
        const q = [idx]; vis[idx] = 1; let h = 0;
        while (h < q.length) {
          const ci = q[h++], cx = ci % this.img.width, cy = ci / this.img.width | 0;
          if (cx < mnX) mnX = cx; if (cx > mxX) mxX = cx; if (cy < mnY) mnY = cy; if (cy > mxY) mxY = cy;
          for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
            const nx = cx + dx, ny = cy + dy;
            if (nx < 0 || nx >= this.img.width || ny < 0 || ny >= this.img.height) continue;
            const ni = ny * this.img.width + nx;
            if (!vis[ni] && !isBg(nx, ny)) { vis[ni] = 1; q.push(ni); }
          }
        }
        if (mxX - mnX > 3 && mxY - mnY > 3 && q.length >= 12) blobs.push({ x: mnX, y: mnY, w: mxX - mnX + 1, h: mxY - mnY + 1 });
      }
    }
    ...
    for (let i = 0; i < blobs.length; i++) {
      ...
      for (let j = i + 1; j < blobs.length; j++) {
        ...
        if (r.x < b.x + b.w && r.x + r.w > b.x && r.y < b.y + b.h && r.y + r.h > b.y) {
          const nx = Math.min(r.x, b.x), ny = Math.min(r.y, b.y);
          r.w = Math.max(r.x + r.w, b.x + b.w) - nx; r.h = Math.max(r.y + r.h, b.y + b.h) - ny;
          r.x = nx; r.y = ny; used[j] = 1;
        }
      }
      merged.push(r);
    }
    ...
    this.frames = new TrackedArray(merged.map((f, i) => ({
      ...
      w: Math.min(this.img.width, f.w + pad * 2), h: Math.min(this.img.height, f.h + pad * 2)
    })));
  ...
  setAlign(code) {
    ...
    ['sbAG', 'mAG'].forEach(id => {
      ...
      if (!g) return;
      g.querySelectorAll('.ab').forEach((b, i) => b.classList.toggle('on', i === idx));
    });
    if (this.scCache) this.buildSheet();
  }
  ...
  autoSuggest() {
    if (!this.frames.length) return;
    const mxW = Math.max(...this.frames.map(f => f.w)), mxH = Math.max(...this.frames.map(f => f.h));
    const snap = (v) => { for (const s of [8, 16, 32, 48, 64, 96, 128, 192, 256, 512]) if (s >= v) return s; return 512; };
    const sw = snap(mxW), sh = snap(mxH);
    document.getElementById('mCW').value = sw; document.getElementById('mCH').value = sh;
    document.getElementById('sCW').value = sw; document.getElementById('sCH').value = sh;
    const cols = Math.max(1, Math.min(16, Math.ceil(Math.sqrt(this.frames.length))));
    document.getElementById('mCols').value = cols; document.getElementById('sCols').value = cols;
    this.buildSheet();
  }
  ...
  buildSheet() {
    ...
    this.frames.forEach((f, i) => {
      ...
      if (this.scaleMode === 'stretch') {
        ...
        const sc = Math.max(cw / f.w, ch / f.h), dw = f.w * sc, dh = f.h * sc;
        const ox = this.alignOff(cw, dw, hCode), oy = this.alignOff(ch, dh, vCode);
        oc.save(); oc.beginPath(); oc.rect(cx, cy, cw, ch); oc.clip();
        oc.drawImage(this.img, f.x, f.y, f.w, f.h, cx + ox, cy + oy, dw, dh); oc.restore();
      } else if (this.scaleMode === 'none') {
        const ox = this.alignOff(cw, f.w, hCode), oy = this.alignOff(ch, f.h, vCode);
        oc.save(); oc.beginPath(); oc.rect(cx, cy, cw, ch); oc.clip();
        oc.imageSmoothingEnabled = false;
        oc.drawImage(this.img, f.x, f.y, f.w, f.h, cx + ox, cy + oy, f.w, f.h); oc.restore();
      } else {
        const sc = Math.min(cw / f.w, ch / f.h), dw = Math.round(f.w * sc), dh = Math.round(f.h * sc);
        const ox = this.alignOff(cw, dw, hCode), oy = this.alignOff(ch, dh, vCode);
  ...
  renderPv(off, shW, shH, cols, rows, cw, ch, gap) {
    ...
    if (!wrap) return;
    const maxW = (wrap.clientWidth || 600) - 20, maxH = (wrap.clientHeight || 380) - 20;
    this.pvZ = this.clamp(Math.min(maxW / shW, maxH / shH, 2), 0.05, 4);
    const pc = document.getElementById('pvCanvas');
    if (!pc) return;
    pc.width = Math.round(shW * this.pvZ); pc.height = Math.round(shH * this.pvZ);
    const pctx = pc.getContext('2d');
    pctx.save(); pctx.scale(this.pvZ, this.pvZ); pctx.imageSmoothingEnabled = false;
    pctx.drawImage(off, 0, 0);
    pctx.strokeStyle = 'rgba(201,123,255,.4)'; pctx.lineWidth = 0.5 / this.pvZ;
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        if (c + r * cols >= this.frames.length) break;
        const cx = c * (cw + gap), cy = r * (ch + gap);
        pctx.strokeRect(cx + 0.25 / this.pvZ, cy + 0.25 / this.pvZ, cw - 0.5 / this.pvZ, ch - 0.5 / this.pvZ);
  ...
  updateStats(shW, shH, cols, rows, cw, ch) {
    ...
    if (el('st-mx')) el('st-mx').textContent = Math.max(...this.frames.map(f => f.w)) + '×' + Math.max(...this.frames.map(f => f.h));
    if (el('st-mn')) el('st-mn').textContent = Math.min(...this.frames.map(f => f.w)) + '×' + Math.min(...this.frames.map(f => f.h));
    ...
    if (el('pvZLbl')) el('pvZLbl').textContent = 'zoom: ' + Math.round(this.pvZ * 100) + '%';
    const tot = cols * rows, pct = Math.round(this.frames.length / tot * 100);
    if (el('pfill')) el('pfill').style.width = pct + '%';
    if (el('plbl')) el('plbl').textContent = `${this.frames.length}/${tot} celdas (${pct}%)`;
  }
  ...
  changePvZ(dir) {
    ...
    const rows = Math.ceil(this.frames.length / cols);
    this.renderPv(this.scCache, this.scCache.width, this.scCache.height, cols, rows, cw, ch, gap);
  }

  @action
  openScale() {
    if (!this.frames.length) { alert('Define frames primero'); return; }
    document.getElementById('mCW').value = document.getElementById('sCW').value;
    document.getElementById('mCH').value = document.getElementById('sCH').value;
    document.getElementById('mCols').value = document.getElementById('sCols').value;
    document.getElementById('mGap').value = document.getElementById('sGap').value;
  ...
  @action
  closeScale() { document.getElementById('scModal')?.classList.remove('open'); }
  ...
  genSheet(loadInEditor) {
    ...
    const rows = Math.ceil(this.frames.length / cols);
    const json = {
      meta: { app: 'SpriteForge', image: name + '.png', format: 'RGBA8888', size: { w: off.width, h: off.height }, scale: 1, frameSize: { w: cw, h: ch } },
      frames: {}
    };
    this.frames.forEach((f, i) => {
      const col = i % cols, row = Math.floor(i / cols);
      json.frames[f.name] = {
        ...
        rotated: false, trimmed: false, spriteSourceSize: { x: 0, y: 0, w: cw, h: ch },
        sourceSize: { w: cw, h: ch }, anchor: { x: 0.5, y: 0.5 }
      };
    });
    if (loadInEditor) {
      off.toBlob((blob) => {
        ...
        const ni = new Image();
        ni.onload = () => {
          this.img = ni; this.imgW = ni.width; this.imgH = ni.height; this.imgName = name;
          this.frames.forEach((f, i) => {
            const col = i % cols, row = Math.floor(i / cols);
            f.x = col * (cw + gap); f.y = row * (ch + gap); f.w = cw; f.h = ch;
          });
          this.closeScale();
          this.fitZoom();
      ...
      setTimeout(() => this.dlText(name + '.json', JSON.stringify(json, null, 2), 'application/json'), 300);
      this.closeScale();
    }
  }
  ...
  buildPayload(f) {
    ...
    const iw = this.img ? this.img.width : 0, ih = this.img ? this.img.height : 0;
    if (f === 'phaser') {
      const obj = { meta: { app: 'SpriteForge', image: name + '.png', format: 'RGBA8888', size: { w: iw, h: ih }, scale: 1 }, frames: {} };
      this.frames.forEach(fr => {
        obj.frames[fr.name] = { frame: { x: fr.x, y: fr.y, w: fr.w, h: fr.h }, rotated: false, trimmed: false, spriteSourceSize: { x: 0, y: 0, w: fr.w, h: fr.h }, sourceSize: { w: fr.w, h: fr.h }, anchor: { x: 0.5, y: 0.5 } };
      });
      return { ext: '.json', mime: 'application/json', text: JSON.stringify(obj, null, 2) };
    }
    if (f === 'array') {
      return { ext: '.json', mime: 'application/json', text: JSON.stringify({ meta: { image: name + '.png', size: { w: iw, h: ih } }, frames: this.frames.map(fr => ({ name: fr.name, x: fr.x, y: fr.y, w: fr.w, h: fr.h })) }, null, 2) };
    }
    if (f === 'pixi') {
      const obj = { frames: {}, meta: { image: name + '.png', size: { w: iw, h: ih }, scale: '1' } };
      this.frames.forEach(fr => {
        obj.frames[fr.name] = { frame: { x: fr.x, y: fr.y, w: fr.w, h: fr.h }, spriteSourceSize: { x: 0, y: 0, w: fr.w, h: fr.h }, sourceSize: { w: fr.w, h: fr.h }, rotated: false, trimmed: false };
      });
      return { ext: '.json', mime: 'application/json', text: JSON.stringify(obj, null, 2) };
    }
    if (f === 'css') {
  ...
  updateExpCode() {
    ...
    if (!expCode) return;
    if (!this.frames.length) { expCode.textContent = '// sin frames'; return; }
    const p = this.buildPayload(this.fmt);
    expCode.textContent = p.text.slice(0, 700) + (p.text.length > 700 ? '\n...' : '');
  }

  @action
  openExport() {
    if (!this.frames.length) { alert('Define al menos un frame'); return; }
    this.updateExpCode();
    document.getElementById('expModal').classList.add('open');
  }

  @action
  closeExport() { document.getElementById('expModal')?.classList.remove('open'); }
  ...
  exportSheet() {
    if (!this.img) { alert('Carga una imagen primero'); return; }
    const off = document.createElement('canvas');
    off.width = this.img.width; off.height = this.img.height;
    const oc = off.getContext('2d'); oc.drawImage(this.img, 0, 0);
    this.frames.forEach(f => {
      oc.strokeStyle = 'rgba(57,232,176,.9)'; oc.lineWidth = 1;
      oc.strokeRect(f.x + 0.5, f.y + 0.5, f.w - 1, f.h - 1);
      oc.font = '9px monospace'; oc.fillStyle = 'rgba(57,232,176,.95)';
      oc.fillText(f.name, f.x + 2, f.y + 10);
    });
    off.toBlob((blob) => {
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a'); a.href = url; a.download = (this.imgName || 'spritesheet') + '_annotated.png'; a.click();
      setTimeout(() => URL.revokeObjectURL(url), 3000);
    }, 'image/png');
  }
